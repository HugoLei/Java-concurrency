...
写作思路： 1. 逻辑推理\(先说结论，再举证\) 2. 实现方式 3. 使用示例

Java SE 1.6对 synchronized 进行了多种优化。 1. 自旋 2. 锁粗化 3. 锁消除 4. 偏向锁 - 轻量级锁 - 重量级锁
...
在 JVM 中每个对象都有个`头` 数组对象：JVM 用3个字宽 word 来存对象头 非数组对象：JVM 用2个字宽 word 存对象头
...
![](../.gitbook/assets/mark-word.png)

> 下面根据锁的四大问题分别进行说明。[《关于锁》](../java-bing-fa-bian-cheng-ji-chu/guan-yu-suo.md)
...
synchronized 代码块的开始处织入 monitorenter synchronized 代码块的结束处织入 monitorexit 线程运行至 monitorenter 时，查找对象头里的重量级锁的指针，后续就进入 OS 的控制范围内。
...
如何避免重量级锁的用户态切换内核态，线程立即挂起后唤醒呢？ 答：当发现有人已经占用锁时，进入`自旋`（自己做点别的事情，时不时地继续判断是否有人占着锁）
...
![](../.gitbook/assets/qing-liang-ji-suo.png)

> 如果关闭了 JVM 偏向锁，则默认从轻量级锁开始，如有竞争则膨胀到重量级锁

![](../.gitbook/assets/qing-liang-ji-suo-+-peng-zhang.png)
...
> 1. 针对上述场景特别优化
> 2. 偏向的意思是：锁偏向第一个获取它的线程
> 3. 这里重点强调的是，在一个线程重复进入锁的情况下，其加锁和解锁不需要额外的 CAS 操作。其实它第一次获取锁也是需要 CAS的。

![](../.gitbook/assets/pian-xiang-suo.png)

**偏向锁的释放：当出现竞争时，需要等待全局安全点（stop the world）**

| 种类 | 优点 | 缺点 | 适用场景 |
| :--- | :--- | :--- | :--- |
| 偏向锁 | 加锁解锁不需要额外的操作，性能好，和执行非同步方法比仅存在纳秒级的差距 | 如果出现锁竞争，会带来额外的锁撤销的消耗 | 适用于只有一个线程访问同步块的场景 |
| 轻量级锁 | 竞争的线程自旋不阻塞，提高了程序的响应速度 | 自旋过程中若无法获取锁，则自旋消耗了额外的 CPU | 追求响应时间，同步执行速度非常快（锁定的时间比较短，小于自旋的周期） |
| 重量级锁 | 直接阻塞不自旋，不消耗 CPU | 线程阻塞，响应时间缓慢 | 追求吞吐量，同步块执行时间较长（与轻量级锁对比） |
...
《浅谈偏向锁、轻量级锁、重量级锁》[https://www.jianshu.com/p/36eedeb3f912](https://www.jianshu.com/p/36eedeb3f912)
